...
export function FolderRow({ folder, algsets, onLongPress, onLongPressAlgSet }: FolderRowProps) {
  ...
  return (
    <View className="flex w-full flex-col gap-2">
      <Animated.View
        style={pressStyle}
        className={`flex min-h-20 w-full flex-row  justify-between rounded-2xl px-3 py-3 ${selectedChild ? 'bg-accent-light' : 'bg-white'}`}>
        <Pressable
          ...
          {scramble !== null ? (
            <DrawScramble
              scramble={scramble ? invertAlgorithm(sanitiseAlgorithm(scramble)) : ''}
              scale={0.6}
              event={previewEvent}
            />
          ) : (
